...
#   for_each         = merge([for app in keys(aws_lb_target_group.gwlbe_ingress_spoke_local_app_alb_tg) : { for zone in module.gwlbe_ingress_spoke_instance : "${app}-${zone.instance_id}" => { app = app, instance_id = zone.instance_id } }]...)
